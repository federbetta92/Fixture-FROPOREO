...
function init() {
  ...
  try {
    const saved = localStorage.getItem('froporeo_view');
    if (saved) { setView(saved); return; }
  } catch(e){}
  if (window.innerWidth < 900) { setView('mobile'); return; }
  if (window.innerWidth >= 900) {
    document.querySelector('.vc-btn[onclick*="desktop"]').style.border = '2px solid var(--orange)';
  }
  setupPWA();
}
